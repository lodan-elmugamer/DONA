# ============================================================
# Script: rank_cellphonedb.R
#
# Purpose:
# This script extracts and ranks focused CellPhoneDB interactions
# within each condition. When exactly two conditions are present,
# it also compares their presence between conditions.
#
# Output:
# Results are saved to:
# the folder set in config.R/comparison/cellphonedb
# ============================================================
...
# set folders
cellphonedb_file <- file.path(results_folder, "cellphonedb/csv/cellphonedb_combined_significant_results.csv")
comparison_folder <- file.path(results_folder, "comparison/cellphonedb")
dir.create(comparison_folder, recursive = TRUE, showWarnings = FALSE)

input_data <- read.csv(cellphonedb_file, stringsAsFactors = FALSE, check.names = FALSE)

# extract and rank one condition
prepare_data <- function(input_table, condition_name) {
  input_table %>% filter(condition == condition_name, pvalue < 0.05) %>% separate(cell_pair_raw, into = c("source", "target"), sep = "\\|", remove = FALSE) %>% mutate(source = trimws(source), target = trimws(target), ligand = trimws(ligand), receptor = trimws(receptor), score = as.numeric(score), pvalue = as.numeric(pvalue)) %>% filter(keep_focused_pairs(source, target)) %>% group_by(source, target, ligand, receptor) %>% summarise(score = max(score, na.rm = TRUE), pvalue = min(pvalue, na.rm = TRUE), .groups = "drop") %>% mutate(condition = condition_name, interaction_id = paste(source, target, ligand, receptor, sep = " | "), cell_pair = paste(source, target, sep = " -> "), ligand_receptor = paste(ligand, receptor, sep = " -> ")) %>% arrange(desc(score)) %>% mutate(rank = row_number(), percentile = percent_rank(score))
}
...
# save ranked results
write.csv(ranked_results, file.path(comparison_folder, "cellphonedb_ranked_interactions.csv"), row.names = FALSE)
...
if (n_conditions == 2) {
  ...
  write.csv(comparison, file.path(comparison_folder, "cellphonedb_condition_comparison.csv"), row.names = FALSE)
}
...
interaction_plot <- ggplot(top_interactions, aes(x = reorder(plot_label, score), y = score)) + geom_col() + coord_flip() + facet_wrap(~condition, scales = "free_y") + theme_classic() + theme(axis.text.y = element_text(size = 7), plot.title = element_text(hjust = 0.5)) + labs(title = "Top CellPhoneDB interactions within each condition", x = "Ligand receptor pair | Sender receiver pair", y = "CellPhoneDB interaction score")

ggsave(file.path(comparison_folder, "cellphonedb_top_ranked_interactions.png"), plot = interaction_plot, width = 14, height = 10, dpi = 300)
